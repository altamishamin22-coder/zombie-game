...
import {
  type Achievement, type BladeId, type DailyChallengeDef, type Difficulty, type DifficultyId,
  type Entity, type EntityKind, type FloatLabel, type Particle, type PowerKind, type SavedData,
  type Screen, type TrailPoint,
  achievements, blades, defaultSave, difficulties, formatNumber, getBlade, getDifficulty,
  getTodaysChallenge, readSave, saveGame,
} from '@/lib/gameData';
import { haptics, resumeAudio, setHapticsEnabled, setMuted, sfx, startMusic, stopMusic } from '@/lib/audio';
...
function freshState(maxLives: number, cfg: Difficulty): GameState {
  return {
    ...
    runCoins: 0, dailyProgress: 0, cfg,
  };
}

function maxLivesFor(bladeId: BladeId, difficultyId: DifficultyId = 'normal') {
  const base = bladeId === 'void' ? 4 : 3;
  return Math.max(1, base + getDifficulty(difficultyId).livesDelta);
}
...
function InstallPrompt({ visible }: { visible: boolean }) {
  ...
  useEffect(() => {
    ...
    // Any phone (iOS, Android, or any other mobile browser) should see the
    // install popup — not just browsers that fire the native
    // beforeinstallprompt event (which excludes Safari, Firefox, Samsung
    // Internet, in-app browsers, etc). Those fall back to manual steps.
    ...
    const timer = window.setTimeout(() => { if (onMobile || deferredPromptRef.current) setShowPrompt(true); }, 1200);
    return () => {
  ...
  const install = async () => {
    if (!deferredPromptRef.current) { setShowPrompt(false); setShowInstructions(true); return; }
...
function DifficultySelector({ save, setSave }: { save: SavedData; setSave: (value: SavedData) => void }) {
  const choose = (id: DifficultyId) => {
    if (id === save.difficulty) return;
    sfx.uiClick();
    haptics.tap();
    const next = { ...save, difficulty: id };
    setSave(next); saveGame(next);
  };
  return (
    <div className="animate-rise-in" style={{ animationDelay: '200ms' }}>
      <div className="mb-2 font-mono-app text-[9px] uppercase tracking-[.24em] text-[#71847a]">Broadcast difficulty</div>
        ...
        {difficulties.map((d) => {
          ...
          return (
            <button
              ...
              onClick={() => choose(d.id)}
              title={d.detail}
              className="border px-4 py-2 font-display text-sm font-bold tracking-[.1em] transition"
              style={active ? { borderColor: d.color, color: d.color, backgroundColor: `${d.color}14`, boxShadow: `0 0 16px ${d.color}33` } : { borderColor: '#30433c', color: '#8da095', backgroundColor: 'transparent' }}
            >
              {d.label}
            </button>
          );
      ...
      <div className="mobile-menu-shell relative z-10 mx-auto flex min-h-[100dvh] max-w-[1440px] flex-col px-6 py-6 sm:px-10 sm:py-8">
        <header className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-5">
            <div className="hidden text-right sm:block">
              <div className="font-mono-app text-[9px] tracking-[.23em] text-[#71847a]">{getDifficulty(save.difficulty).label} RECORD</div>
              <div className="font-display text-2xl font-bold text-[#f3f2df]">{formatNumber(save.highScores[save.difficulty])}</div>
            </div>
        ...
        <section className="mobile-menu-content flex flex-1 items-center py-14 sm:py-20">
          <div className="max-w-2xl">
            ...
            <div className="mt-8">
              <DifficultySelector save={save} setSave={setSave} />
            </div>
            <div className="mt-8 flex flex-wrap items-center gap-4 animate-rise-in" style={{ animationDelay: '240ms' }}>
            ...
            <div className="mt-8 flex items-center gap-8 animate-rise-in" style={{ animationDelay: '360ms' }}>
              <StatChip icon={<Trophy className="h-4 w-4" />} label="best run" value={formatNumber(save.highScores[save.difficulty])} />
...
function Settings({ save, setSave, setScreen, screenFx, setScreenFx, swipeAssist, setSwipeAssist }: {
  ...
  const toggleHaptics = () => {
    ...
    setHapticsEnabled(next.hapticsOn);
    setSave(next); saveGame(next);
    sfx.uiClick();
    if (next.hapticsOn) haptics.tap();
  };
  return (
    <main className="noise-overlay relative min-h-[100dvh] overflow-hidden bg-[#09100f]">
      <CityBackdrop dense />
      <div className="relative z-10 mx-auto min-h-[100dvh] max-w-3xl px-6 py-6 sm:px-10 sm:py-10">
        ...
        <div className="mt-14 border border-[#30433c] bg-[#0d1717]/90 p-6 sm:p-10">
          ...
          <div className="mt-10 divide-y divide-[#30433c] border-y border-[#30433c]">
            ...
            <button type="button" data-testid="button-toggle-haptics" onClick={toggleHaptics} className="flex w-full items-center justify-between py-6 text-left transition hover:bg-[#162218]/70">
              <div className="flex items-center gap-4"><Zap className={save.hapticsOn ? 'text-[#d3ff35]' : 'text-[#71847a]'} /><div><div className="font-display text-xl font-bold tracking-[.08em] text-[#f3f2df]">HAPTICS</div><div className="font-mono-app text-[10px] text-[#71847a]">Vibration on hits and hazards (phones only)</div></div></div>
              <div className={`h-6 w-11 border p-1 transition ${!save.hapticsOn ? 'border-[#4a5149] bg-[#202a25]' : 'border-[#d3ff35] bg-[#d3ff35]/20'}`}><div className={`h-4 w-4 transition ${save.hapticsOn ? 'translate-x-5 bg-[#d3ff35]' : 'bg-[#71847a]'}`} /></div>
            </button>
          </div>
...
function Game({ save, setSave, setScreen, screenFx, swipeAssist, onRunEnd }: {
  save: SavedData; setSave: (value: SavedData) => void; setScreen: (screen: Screen) => void; screenFx: boolean; swipeAssist: boolean;
  onRunEnd: (result: { score: number; difficulty: DifficultyId; isRecord: boolean }) => void;
}) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const arenaRef = useRef<HTMLDivElement | null>(null);
  const blade = useMemo(() => getBlade(save.selected), [save.selected]);
  const difficultyCfg = useMemo(() => getDifficulty(save.difficulty), [save.difficulty]);
  const startingLives = maxLivesFor(save.selected, save.difficulty);
  const stateRef = useRef<GameState>(freshState(startingLives, difficultyCfg));
  ...
  // Keep the screen awake during a run, and auto-pause instead of letting the
  // player take unfair hits if they switch apps or lock their phone.
  useEffect(() => {
    ...
    const requestWakeLock = async () => {
      try {
        ...
        // wake lock unavailable — not critical, the game still works
      }
    };
    requestWakeLock();
    const onVisibility = () => {
      if (document.hidden) {
        if (!pausedRef.current && !stateRef.current.gameOver) { pausedRef.current = true; setPaused(true); }
      } else if (!wakeLock) {
        requestWakeLock();
      }
    };
  ...
  const spawnEntity = useCallback((s: GameState) => {
    ...
    const bombChance = Math.min(0.4, (0.09 + s.wave * 0.007) * s.cfg.bombMult);
    ...
    const dx = targetX - x;
    const dy = targetY - y;
    const baseSpeed = (kind === 'runner' ? 1.45 : kind === 'brute' ? 0.68 : kind === 'bomb' ? 0.82 : kind === 'fruit' ? 0.72 : kind === 'survivor' ? 1.1 : kind === 'powerup' ? 0.6 : 0.98) * s.cfg.speedMult;
  ...
  const handleSlice = useCallback((x: number, y: number) => {
    ...
    const scoreMult = (multiActive ? 2 : 1) * s.cfg.scoreMult;
    ...
    s.entities.forEach((entity) => {
      ...
      if (entity.kind === 'boss') {
        ...
        if ((entity.hp ?? 0) <= 0) {
          ...
          if (saveRef.current.difficulty === 'nightmare') {
            const result = unlockAchievements(saveRef.current, ['nightmare_boss']);
      ...
      if (entity.kind === 'fruit') {
        ...
        const cap = maxLivesFor(blade.id, saveRef.current.difficulty);
  ...
  useEffect(() => {
    ...
    const finalizeRun = () => {
      ...
      const previousBest = current.highScores[current.difficulty] ?? 0;
      const isRecord = s.score > previousBest;
      let next: SavedData = {
        ...current,
        highScore: Math.max(current.highScore, s.score),
        highScores: { ...current.highScores, [current.difficulty]: Math.max(previousBest, s.score) },
      ...
      onRunEnd({ score: s.score, difficulty: current.difficulty, isRecord });
      setScreen('gameover');
    };
    ...
    const tick = (now: number) => {
      ...
      if (!pausedRef.current && !s.gameOver) {
        ...
        if (s.spawn <= 0 && !s.bossActive) {
          spawnEntity(s);
          s.spawn = (Math.max(22, 54 - s.wave * 3) + Math.random() * 22) / s.cfg.spawnMult;
        }
        const nextWave = Math.floor((s.elapsed * s.cfg.waveMult) / 16) + 1;
        if (nextWave > s.wave) {
          s.wave = nextWave;
          sfx.waveUp();
          haptics.tap();
          if (s.wave >= 10 && (saveRef.current.difficulty === 'hard' || saveRef.current.difficulty === 'nightmare')) {
            const result = unlockAchievements(saveRef.current, ['hard_wave_10']);
            if (result.unlocked.length) { saveRef.current = result.save; setSave(result.save); saveGame(result.save); pushToasts(result.unlocked); }
          }
        }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [blade.color, blade.glow, blade.id, burst, dailyDef, onRunEnd, pushToasts, screenFx, setSave, setScreen, spawnBoss, spawnEntity]);
  ...
  const restart = () => {
    const lives = maxLivesFor(save.selected, save.difficulty);
    stateRef.current = freshState(lives, difficultyCfg);
...
function GameOver({ save, setScreen, lastRun }: {
  save: SavedData; setScreen: (screen: Screen) => void;
  lastRun: { score: number; difficulty: DifficultyId; isRecord: boolean } | null;
}) {
  useEffect(() => { sfx.gameOver(); haptics.gameOver(); }, []);
  const { def } = useMemo(() => getTodaysChallenge(), []);
  const dailyDone = save.daily.challengeId === def.id && save.daily.completed;
  const runDifficulty = getDifficulty(lastRun?.difficulty ?? save.difficulty);
  return (
    <main className="noise-overlay relative min-h-[100dvh] overflow-hidden bg-[#09100f]">
      <CityBackdrop dense />
      <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-3xl flex-col items-center justify-center px-6 py-12 text-center">
        <div className="flex items-center gap-2 font-mono-app text-[10px] uppercase tracking-[.38em] text-[#ff8247]">
          Signal lost / run terminated
          <span className="border px-2 py-0.5" style={{ borderColor: runDifficulty.color, color: runDifficulty.color }}>{runDifficulty.label}</span>
        </div>
        ...
        {lastRun && (
          <div className="mt-6 flex items-center gap-3">
            <div data-testid="text-final-score" className="font-display text-5xl font-black text-[#f3f2df]">{formatNumber(lastRun.score)}</div>
            {lastRun.isRecord && <span className="border border-[#d3ff35]/60 bg-[#162218]/80 px-2 py-1 font-mono-app text-[9px] uppercase tracking-[.18em] text-[#d3ff35]">New record</span>}
          </div>
        )}
        <div className="mt-8 grid w-full max-w-lg grid-cols-3 border-y border-[#30433c] py-5">
          <div><div className="font-mono-app text-[9px] uppercase tracking-[.18em] text-[#71847a]">{runDifficulty.label} record</div><div className="mt-1 font-display text-3xl font-bold text-[#d3ff35]">{formatNumber(save.highScores[runDifficulty.id])}</div></div>
...
function App() {
  ...
  const [lastRun, setLastRun] = useState<{ score: number; difficulty: DifficultyId; isRecord: boolean } | null>(null);
  ...
  return (
    <div className="min-h-[100dvh] bg-[#09100f]">
      <ScreenFade screenKey={screen}>
        ...
        {screen === 'game' && <Game save={save} setSave={setSave} setScreen={setScreen} screenFx={screenFx} swipeAssist={swipeAssist} onRunEnd={setLastRun} />}
        {screen === 'gameover' && <GameOver save={save} setScreen={setScreen} lastRun={lastRun} />}
      </ScreenFade>
